...
export class UrlBuilder {
    ...
    buildUrlsFromPackage(project, projectChild) {
        if (project.documents) {
            project.documents.forEach((document) => {
                this.buildUrlsForDocument(document);
            });
    ...
    applyAnchorUrl(reflection, containerUrl) {
        ...
        if (anchorId) {
            ...
            let anchorParts = [];
            if (reflection.parent?.parent?.kind === ReflectionKind.Property &&
        ...
        if (this.options.getValue('outputFileStrategy') === OutputFileStrategy.Members) {
            reflection.traverse((child) => {
    ...
    getAnchorName(reflection) {
        ...
        if (typeParams?.length) {
            anchorParts.push(typeParams?.map((typeParameter) => typeParameter.name).join('-'));
        }
